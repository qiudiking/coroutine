...
use Scar\db\Query;
use Scar\exception\ValidateException;
use Scar\model\Collection as ModelCollection;
use Scar\model\Relation;
use Scar\model\relation\BelongsTo;
use Scar\model\relation\BelongsToMany;
use Scar\model\relation\HasMany;
use Scar\model\relation\HasManyThrough;
use Scar\model\relation\HasOne;
use Scar\model\relation\MorphMany;
use Scar\model\relation\MorphOne;
use Scar\model\relation\MorphTo;
...
{
	// 数据库查询对象池
	protected static $links = 'Model_links';
	// 数据库配置
	...
	public function readMaster($all = false)
	{
		...
		Coroutine::getContext()[static::$readMaster][$model] = true;
		return $this;
	}
	...
	protected function buildQuery()
	{
		...
		if (isset(Coroutine::getContext()[static::$readMaster]['*']) || isset(Coroutine::getContext()[static::$readMaster][$this->class])) {
			$query->master(true);
		}
	...
	public function getQuery($buildNewQuery = false)
	{
		...
		} elseif (!isset(  Coroutine::getContext()[self::$links][$this->class])) {
			// 创建模型查询对象
			Coroutine::getContext()[self::$links][$this->class] = $this->buildQuery();
		}

		return Coroutine::getContext()[self::$links][$this->class];
	}
	...
	protected function initialize()
	{
		...
		if (!isset(Coroutine::getContext()[static::$initialized][$class])) {
			Coroutine::getContext()[static::$initialized][$class] = true;
			static::init();
		}
	}
	 ...
	 * 转换当前模型数据集为数据集对象
	 * @access public
	 * @param array|\Scar\Collection $collection 数据集
	 * @return \Scar\Collection
	 */
	...
	public static function event($event, $callback, $override = false)
	{
		...
		if ($override) {
			Coroutine::getContext()[self::$event][$class][$event] = [];
		}
		Coroutine::getContext()[self::$event][$class][$event][] = $callback;
	}
	...
	protected function trigger($event, &$params)
	{
		...
		if (isset( Coroutine::getContext()[self::$event][$this->class][$event])) {
			foreach (Coroutine::getContext()[self::$event][$this->class][$event] as $callback) {
