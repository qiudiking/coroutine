...
use Scar\db\exception\DataNotFoundException;
use Scar\db\exception\ModelNotFoundException;
use Scar\Exception;
use Scar\db\exception\DbException;
use Scar\Loader;
use Scar\Model;
use Scar\model\relation\OneToOne;
...
class Query
{
    ...
    public function getTable($name = '')
    {
        if ($name || empty($this->table)) {
            ...
            if ($name) {
                $tableName .= strtolower(trim(preg_replace("/[A-Z]/", "_\\0", $name), "_"));
            }
        } else {
     ...
     * @return mixed
     */
    public function value($field, $default = null, $force = false,$result_field = false)
    {
        $result = false;
        ...
        if (false === $result) {
            ...
            $result = isset( $arr[$field] )? $arr[$field] : false;
            if ($force) {
                $result += 0;
            }
    ...
    public function count($field = '*')
    {
        if (isset($this->options['group'])) {
            ...
            return $this->table([$subSql => '_group_count_'])->value('COUNT(*) AS tp_count', 0, true,'tp_count');
        }

        return $this->value('COUNT(' . $field . ') AS tp_count', 0, true,'tp_count');
    }
    ...
    {
        return $this->value('SUM(' . $field . ') AS tp_sum', 0, true,'tp_sum');
    }
    ...
    {
        return $this->value('MIN(' . $field . ') AS tp_min', 0, $force,'tp_min');
    }
    ...
    {
        return $this->value('MAX(' . $field . ') AS tp_max', 0, $force,'tp_max');
    }
    ...
    {
        return $this->value('AVG(' . $field . ') AS tp_avg', 0, true,'tp_avg');
    }
    ...
    public function paginate($listRows = null, $simple = false, $config = [])
    {
        ...
        /** @var Paginator $class */
        $class = false !== strpos($config['type'], '\\') ? $config['type'] : '\\Scar\\paginator\\driver\\' . ucwords($config['type']);
    ...
    public function getTableInfo($tableName = '', $fetch = '')
    {
        ...
        if (!isset(Coroutine::getContext()[self::$info][$db . '.' . $guid])) {
            if (!strpos($guid, '.')) {
                $schema = $db . '.' . $guid;
            } else {
                $schema = $guid;
           ...
           /* if (!App::$debug && is_file(RUNTIME_PATH . 'schema/' . $schema . '.php')) {
	        ...
	        Coroutine::getContext()[self::$info][$db . '.' . $guid] = ['fields' => $fields, 'type' => $type, 'bind' => $bind, 'pk' => $pk];



        }
        return $fetch ? Coroutine::getContext()[self::$info][$db . '.' . $guid][$fetch] :  Coroutine::getContext()[self::$info][$db . '.' . $guid];
    }
	 ...
	 *
	 * 执行查询但只返回Statement对象
	 * @return \Swoole\Coroutine\MySQL\Statement|string
	 * @throws \Exception
	 */
     ...
     * @return Collection|false|\Swoole\Coroutine\MySQL\Statement|string
     * @throws DbException
    ...
    public function select($data = null)
    {
        ...
        if (false === $resultSet) {
            ...
            if ($resultSet = $this->trigger('before_select', $options)) {
                ...
                if ($resultSet instanceof Coroutine\MySQL\Statement) {
                    // 返回PDOStatement对象
                    return $resultSet;
                }
            }
     ...
     * @access public
     * @param array|string|Query|\Closure $data
     * @return array|\Swoole\Coroutine\MySQL\Statement|string|Model
     * @throws DbException
     * @throws ModelNotFoundException
     * @throws DataNotFoundException
     */
    ...
    {
	    Coroutine::getContext()[self::$event][$event] = $callback;
    }
    ...
    {
        $result = false;
        if (isset(Coroutine::getContext()[self::$event][$event])) {
            $callback = Coroutine::getContext()[self::$event][$event];
